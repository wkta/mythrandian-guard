...
class ChallSelectionModel(CogObject):  # it holds the state of missions
    ...
    def claim_reward(self, index):
        if not self.is_m_over(index):
            raise Exception
        self._ongoing_missions[index] = 0  # reset state

        obj_loot = Loot.gen_random(self._difficulties[index])
        self.pev(MyEvTypes.LootReady, idx=index, obj=obj_loot)
        print('sendin loot!')
...
class ChallSelectionView(EventReceiver):
    ...
    def proc_event(self, ev, source=None):
        ...
        elif ev.type == MyEvTypes.LootReady:
        ...
        if ev.type == EngineEvTypes.LOGICUPDATE:
            if len(self._timers) > 0:
                ...
                for mission_idx, v in self._timers.items():
                    dt = time.time() - v
                    if dt > 3.0:
                        self._modele.flag_mission_done(mission_idx)
                        defunct_set.add(mission_idx)
                for elt in defunct_set:
                    del self._timers[elt]
